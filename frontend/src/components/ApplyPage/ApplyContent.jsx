  ...
  return (
    <div className="flex flex-col p-10 gap-8 lg:px-[180px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7 text-white">
        {Data.slice((page*12)-12,page*12).map((item, idx) => {
          return (
            <div key={idx} className="bg-[linear-gradient(90deg,rgba(173,216,230,0.3),rgba(221,160,221,0.5))] p-6 rounded-2xl flex flex-col gap-3">
      ...
      <div>
        <div className="flex gap-5 justify-center align-center">
          <IoIosArrowDropleftCircle color="white" size={35} onClick={()=>handelPage("-")}/>
          <div className="flex flex-row text-white gap-5 items-center">
            {
            [1,2,3,4].map((no,idx)=><span key={idx} onClick={()=>setPage(no)} className={page == no && "bg-[linear-gradient(90deg,rgba(255,216,230,.5),rgba(221,160,221,0.4))] p-1 pl-2 pr-2 rounded-3xl cursor-pointer"}>{no}</span>)
          }
          </div>
          <IoIosArrowDroprightCircle color="white" size={35} onClick={()=>handelPage("+")}/>
        </div>
      </div>
    </div>
  );
